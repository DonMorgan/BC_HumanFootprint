...
#Alpine + Glacier from BTM
AplineG1<-st_read(file.path(spatialOutDir,'disturbance_sfR1.gpkg'))
AlpineG <- AlpineG1 %>%
  dplyr::filter(ID=='1' | ID=='20') %>%
  mutate(AlpineG=1) %>%
  dplyr::select(AlpineG)
write_sf(AlpineG,file.path(spatialOutDir,'AlpineG.gpkg'))

alpineD.1<-AlpineG %>%
  mutate(alpineG_id=as.numeric(rownames(.))) %>%
  mutate(area_Ha=units::set_units(st_area(.),ha)) %>%
  units::drop_units(.) %>%
  dplyr::filter(area_Ha>10) %>%
  mutate(alpineG=1) %>%
  dplyr::select(alpineG_id,alpineG,area_Ha) %>%
  st_cast("POLYGON") %>%
  st_make_valid(.)
#Remove any possible interal mapsheet boundaries
alpineGD<-alpineD.1 %>%
...
#mapview(aplineGD.1)+mapview(aplineGD)
write_sf(alpineGD,file.path(spatialOutDir,'alpineGD.gpkg'))

#Buffering glaciers and reclassifying so that resistance increases inwards of ice - modified from Cesar Estevo
alpineG_file<-file.path(spatialOutDir,'alpineG_buffered.tif')
if (!file.exists(alpineG_file)) {
  alpineG_sf_buf<-buffer_fun(alpineGD)
  alpineG_buffered.tif<-rast(alpineG_sf_buf) %>% max(na.rm=T)
  writeRaster(alpineG_buffered.tif,file.path(spatialOutDir,'alpineG_buffered.tif'),overwrite=T)
}else{
  alpineG_buffered.tif<-rast(file.path(spatialOutDir,'alpineG_buffered.tif'))
}
